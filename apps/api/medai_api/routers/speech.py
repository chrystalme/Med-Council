"""Speech I/O — Whisper transcription + TTS (both Pro-only).

POST /api/speech/transcribe   — multipart audio → text
POST /api/speech/synthesize   — text → mp3 audio bytes

Free tier uses the browser's Web Speech API client-side; these
endpoints are gated behind `require_pro` for the higher-quality
provider flow.
"""
