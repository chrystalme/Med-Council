...
from agent_runtime import (
    ...
    run_agent,  # re-exported so tests can patch main.run_agent
    run_agent_raw,  # re-exported so output_guardrails.py / council.py can call main.run_agent_raw
    set_providers as _set_runtime_providers,
)

# Re-exported for test_output_guardrails.py (constructs synthetic guardrail
# tripwire fixtures referencing the agent objects through main.*).
from council import (  # noqa: F401, E402
    consensus_agent,
    message_agent,
)
...
def _get_db():
    ...
    Re-exported here so `routers.feedback` (and tests that patch it) can hit a
    single seam regardless of which router needs the connection.
    """
    return _db.connect()
...
from routers import patient_email as _patient_email_router  # noqa: E402
...
app.include_router(_patient_email_router.router)
...
# Re-export `run_agent` / `run_agent_raw` so external callers
# (output_guardrails.py, council.py) and tests can patch a single seam.
__all__ = ["app", "run_agent", "run_agent_raw", "_get_db"]
