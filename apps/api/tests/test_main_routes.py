...
class ConsultationsListRouteTest(unittest.TestCase):
    def setUp(self) -> None:
        self.client = _client()
        self.con = _FakeCon()
        from routers import consultations as _consultations_router

        self._patcher = patch.object(_consultations_router._db, "connect", return_value=self.con)
        self._patcher.start()
        self.addCleanup(self._patcher.stop)
...
class AttachmentsListRouteTest(unittest.TestCase):
    ...
    def test_lists_attachments_for_owned_case(self) -> None:
        ...
        with patch("attachments.get_attachment_store", return_value=_StubStore()):
...
class SaveConsultationRouteTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._db_patch = patch.object(_consultations_router._db, "connect", return_value=self.con)
        self._db_patch.start()
        self.addCleanup(self._db_patch.stop)
...
class PatientEmailRouteTest(unittest.TestCase):
    ...
    def test_send_succeeds_when_pro_dependency_overridden(self) -> None:
        import main as _main
        from auth import AuthUser, require_pro
        from routers import patient_email as _patient_email_router
        ...
        _main.app.dependency_overrides[require_pro] = _fake_pro
        try:
            with patch.object(_patient_email_router, "send_patient_email", side_effect=_fake_send):
                r = self.client.post(
                    "/api/patient/email",
                    json={
                        "to": "alice@example.com",
    ...
    def test_503_when_resend_not_configured(self) -> None:
        import main as _main
        from auth import AuthUser, require_pro
        from escalation import ResendNotConfiguredError
        from routers import patient_email as _patient_email_router
...
class ConsultationCapTest(unittest.TestCase):
    ...
    def test_402_when_free_tier_at_cap(self) -> None:
        from routers.consultations import FREE_CONSULTATION_CAP
