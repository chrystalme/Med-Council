"""Patient consultation persistence + vector retrieval (Phase 3).

Owns the consultations table: save a finished session, list all for the
current user, fetch one with its joined case state, delete one, and run
a vector retrieval against the user's stored consultations.

Free-tier callers are capped at FREE_CONSULTATION_CAP via a per-user
advisory lock so two parallel saves can't both squeak past the cap check.
"""
